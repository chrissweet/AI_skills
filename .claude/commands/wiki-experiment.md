...
Full procedure: see `.claude/skills/wiki-experiment.md` and `wiki/AI_skills.wiki/SCHEMA_AI_skills.md`. Summary:
...
7. Update `index_AI_skills.md` with one-line descriptions of new pages.
...
11. **Finish the cycle.** Commit in the wiki's own git repo in two steps, without asking. One commit per log entry keeps `git blame` on the log a faithful per-entry record (see "Log Entry Attribution" in SCHEMA):
    ```
    git -C wiki/AI_skills.wiki add <page-and-index-files-by-name>
    git -C wiki/AI_skills.wiki commit -m "<descriptive message>"
    git -C wiki/AI_skills.wiki add log_AI_skills.md
    git -C wiki/AI_skills.wiki commit -m "log: <descriptive message>"
    ```
