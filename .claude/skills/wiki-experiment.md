...
Follow the Ingest procedure defined in `wiki/AI_skills.wiki/SCHEMA_AI_skills.md`. The steps below are pointers, not a substitute for SCHEMA:
...
7. Update `index_AI_skills.md` with one-line descriptions of new pages in the right category.
8. Append a `## [YYYY-MM-DD] update | Experiment name` entry to `log_AI_skills.md`. The first bullet is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the run. See "Log Entry Attribution" in `SCHEMA_AI_skills.md`.
