---
name: colab
description: Operate Google Colab runtimes via the `colab` CLI — provision GPU/TPU sessions, run Python/shell on a remote VM, sync files, manage auth, export work as a notebook. Use when the user wants to run code on a Colab VM, allocate a session, or troubleshoot Colab CLI auth.
user-invocable: true
allowed-tools:
  - Read
  - Write
